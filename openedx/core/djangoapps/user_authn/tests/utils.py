...
class AuthAndScopesTestMixin(object):
    ...
    def _create_jwt_token(self, user, auth_type, scopes=None, include_org_filter=True, include_me_filter=False):
        ...
        if scopes is None:
            scopes = self.default_required_scopes
    ...
    def _get_response(self, auth_type, requesting_user=None, requested_user=None, url=None, token=None):
        """
    ...
    def test_another_user(self, auth_type, scopes_enforced, mock_log):
        ...
        with ENFORCE_JWT_SCOPES.override(active=scopes_enforced):
            resp = self._get_response(auth_type, requesting_user=self.other_student)
    ...
    def test_jwt_no_scopes(self, auth_type, scopes_enforced, mock_log):
        ...
        with ENFORCE_JWT_SCOPES.override(active=scopes_enforced):
            ...
            resp = self._get_response(AuthType.jwt, token=jwt_token)
    ...
    def test_valid_oauth_token(self):
        resp = self._get_response(AuthType.oauth)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    def test_invalid_oauth_token(self):
        resp = self._get_response(AuthType.oauth, token="fooooooooooToken")
    ...
    def test_expired_oauth_token(self):
        ...
        token.save()
        resp = self._get_response(AuthType.oauth, token=token)
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
